...
void get_updated_state(TESIC_APB_t*ctx){
    ...
    if(ret>0){//get updated state
        ...
        int already_received = recv(sockfd, b8, 4,MSG_WAITALL);
        //printf("already_received=%d\n",already_received);
        //printf("received %u bytes\n",already_received);
        //for(unsigned int i=0;i<already_received;i++){
        //    printf("%02X ",b8[i]);
        //}
        //printf("\n");
...
//assume we are slave
void set_updated_state(TESIC_APB_t*ctx){
    if(ctx->CFG & TESIC_APB_CFG_SLAVE_STS_MASK){
        if(0==(emu_CFG & TESIC_APB_CFG_SLAVE_STS_MASK)){//send data
            ...
            if(0==padded_len) {
                //printf("emu_state=%u\n",emu_state);
                assert((RTPDU_DAT==emu_state)||(CTPDU_DAT==emu_state)||(CTPDU_FINAL_DAT==emu_state));
...
uint32_t emu_rd_cnt(TESIC_APB_t*ctx){
    //assert(ctx->CFG & TESIC_APB_CFG_OWNER_MASK);//slave shall have ownership
    get_updated_state(ctx);
